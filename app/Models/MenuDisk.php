...
class MenuDisk extends Model
{
    ...
    public function donatedByIndividual()
    {
        // The method name is what belongsTo derives the key from, so this
        // name -- not `donatedBy` -- is what makes donated_by_individual_id
        // the default. The owner key on Individual is `id`, also the default.
    ...
    public function getMenusetPageNumberAttribute()
    {
        ...
        $index = DB::table('menu_disks')
            ...
            ->orderBy('number', $this->menu->menuSet->sort_direction)
            ->orderBy('issue', $this->menu->menuSet->sort_direction)
            ->orderBy('version', 'asc')
            ->orderBy('part', 'asc')
            ->get()
            ->pluck('id')

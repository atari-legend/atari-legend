@extends('layouts.app')
@section('title', $review->game->name.' review (Atari ST)')

@if ($review->screenshots->isNotEmpty())
    @section('image', $review->screenshots->first()->getUrlRoute('game', $review->game))
@endif

@section('content')
    <h1 class="visually-hidden">{{ $review->game->name }}</h1>
    <div class="row">

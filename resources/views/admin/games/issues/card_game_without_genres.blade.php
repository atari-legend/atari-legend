<div class="card mb-3 bg-light">
    <div class="card-body p-0">
        @if ($gameWithoutGenre->screenshots->isNotEmpty())
            <div class="clearfix">
                @foreach ($gameWithoutGenre->screenshots->take(4) as $screenshot)
                    <img class="w-50 pixelated float-left" src="{{ asset('storage/images/game_screenshots/'.$screenshot->file) }}" alt="Screenshot of {{ $gameWithoutGenre->game_name }}">
                @endforeach
            </div>
        @endif
        ...
        <form class="px-2" method="post" action="{{ route('admin.games.issues.genres') }}">
            @csrf
            <ul class="list-unstyled mb-3">
            @foreach ($genres as $genre)
                <li class="w-45 d-inline-block">
                    <input type="checkbox" class="form-check-input" id="genre_{{ $genre->id }}">

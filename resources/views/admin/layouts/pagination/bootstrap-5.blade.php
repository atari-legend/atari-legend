        ...
        <ul class="pagination justify-content-center">
            ...
            @foreach ($elements as $element)
                ...
                @if (is_string($element))
                    <li class="page-item disabled border-0" aria-disabled="true"><span class="page-link border-0">{{ $element }}</span></li>
                @endif
                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        @if ($page == $paginator->currentPage())
                            <li class="page-item active d-none d-sm-inline" aria-current="page"><span class="page-link border-0">{{ $page }}</span></li>
                        @else
                            <li class="page-item d-none d-sm-inline"><a class="page-link border-0" href="{{ $url }}">{{ $page }}</a></li>
                        @endif
                    @endforeach
                @endif
            @endforeach

    ...
    <div class="card-body">
        ...
        <form action="{{ isset($content) ? route('admin.menus.disks.content.update', ['disk' => $disk, 'content' => $content]) : route('admin.menus.disks.content.store', ['disk' => $disk]) }}" method="post">
            ...
            @isset ($content)
                @if ($content->game)
                    <p>
                        This content points to the game <a href="{{ route('admin.games.games.edit', $content->game) }}">{{ $content->game->name }}</a>.
                ...
                @elseif ($content->release)
                    <p>
                        ...
                        of <a href="{{ route('admin.games.games.edit', $content->release->game) }}">{{ $content->release->game->name }}</a>.

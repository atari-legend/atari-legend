    ...
    <div class="card-body">
        <form action="{{ isset($set) ? route('admin.menus.sets.update', $set) : route('admin.menus.sets.store') }}" method="post">
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-6">
                    @php
                        // menu_sets.sort_direction is an enum of 'asc' / 'desc' - the
                        ...
                        $sort = old('sort', $set->sort_direction ?? 'asc');
                    @endphp
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('crews') is-invalid @enderror" multiple size="6" name="crews[]" required>
                    @foreach ($crews as $crew)
                        <option value="{{ $crew->getKey() }}" @if (isset($set) && $set->crews->contains($crew)) selected @endif>{{ $crew->name }}</option>
                    @endforeach
                </select>

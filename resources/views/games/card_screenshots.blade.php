                    ...
                    <div class="carousel-thumbnails flex-grow-1 overflow-hidden" style="flex-basis: 0;" data-bs-carousel="carousel-screenshots">
                        @foreach($game->screenshots as $screenshot)
                            <a href="#carousel-screenshots" data-bs-slide-to="{{ $loop->index }}" @if ($loop->first) class="active" @endif>
                                <img class="w-100 mb-2" src="{{ $screenshot->getUrlRoute('game', $game) }}" alt="Thumbnail of other screenshot of {{ $game->name }}" loading="lazy">
                            </a>
                        @endforeach
                    </div>
                </div>
                        ...
                        <div class="carousel-inner">
                            @foreach($game->screenshots as $screenshot)
                                <div class="carousel-item @if ($loop->first) active @endif">
                                    <img class="w-100 d-block pixelated" src="{{ $screenshot->getUrlRoute('game', $game) }}" alt="Large screenshot of {{ $game->name }}" loading="lazy">
                                </div>
                            @endforeach
                        </div>
